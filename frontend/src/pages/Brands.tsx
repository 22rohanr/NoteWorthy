...
import { fragrances } from '@/data/dummyData';
...
export default function Brands() {
  ...
  const fragranceCount = (brandId: string) =>
    fragrances.filter((f) => f.brand.id === brandId).length;
            ...
            {filtered.map((brand) => (
              ...
              >
                ...
                <div className="mt-4 pt-4 border-t border-border/40 text-xs text-muted-foreground">
                  {fragranceCount(brand.id)} fragrance{fragranceCount(brand.id) !== 1 ? 's' : ''} listed
                </div>
              </Link>
            ))}
          </div>
